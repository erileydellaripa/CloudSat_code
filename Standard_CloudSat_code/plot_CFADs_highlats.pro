...
pro plot_CFADs_highlats, FILENAMES = filenames, SORTBY = sortby, SAMPLE_NAME=sample_name, $
                ndays = ndays, nlats=nlats, nlons=nlons, PATH = path
...
;---------------Find, profile by profile "cloud" base, top, and thickness
        h        = hist_2d(xcloud-min(xcloud), zcloud/240) ;;a cloud mask postage stamp in 250m z bins
        if max(h) gt 1 then h[where(h gt 1)] = 1
        ;;create array to be filled
        syze     = size(h)
        base_ind = fltarr(syze[1]) ;x-dimension, width of the cloud
        top_ind  = fltarr(syze[1]) ;z-dimension, height of the cloud*240 km
...
;restore, alpha and lambda, the arrays of # of samples per nlons per ndays for abs(lats) > 60
restore, '/Users/mapesgroup/CLOUDSAT/CLOUDSAT_CODE/Standard_Code/global_loops/Antarctic_samples.sav'
restore, '/Users/mapesgroup/CLOUDSAT/CLOUDSAT_CODE/Standard_Code/global_loops/Arctic_samples.sav'
tot_samples = 0.
if sample_name eq 'Arctic' then begin
  for i = 0, n_elements(lambda)-1 do begin
    samples = lambda[i] * ndays * nlons
    tot_samples = tot_samples + samples    
  endfor 
    cloud_fraction = total(CFAD, 1)/tot_samples
    overlap_cloud_fraction = total(OCFAD, 1)/tot_samples
    total_cloud_fraction = total(TOTALCFAD, 1)/tot_samples
endif else begin
tot_samples = 0.
for i = 0, n_elements(alpha)-1 do begin
    samples = alpha[i] * ndays * nlons
    tot_samples = tot_samples + samples
endfor 
    cloud_fraction = total(CFAD, 1)/tot_samples
    overlap_cloud_fraction = total(OCFAD, 1)/tot_samples
